...
static struct autotest {
    autotest() {
        ...
        CHECK2(_parse_int_literal(cursor, out) && out == 12345);

        cursor = "   -12345     asdlkjfask   ";
        CHECK2(_parse_int_literal(cursor, out) && out == -12345);

        cursor = "0x12345";
        CHECK2(_parse_int_literal(cursor, out) && out == 0x12345);

        cursor = "0xFEDCBA01";
        CHECK2(_parse_int_literal(cursor, out) && out == 0xFEDCBA01);

        cursor = "0b10101100";
        CHECK2(_parse_int_literal(cursor, out) && out == 0b10101100);

        cursor = "01234567";
        CHECK2(_parse_int_literal(cursor, out) && out == 01234567);

        cursor = "not_a_number";
        CHECK2(!_parse_int_literal(cursor, out));

        cursor = "1234badsuffix";
        CHECK2(!_parse_int_literal(cursor, out));

        cursor = " 1234 sometest";
        CHECK2(_parse_int_literal(cursor, out) && out == 1234 && strcmp(cursor, " sometest") == 0);
    }
